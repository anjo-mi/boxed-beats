...
const deleteAuthUsers = async () => {
  try {
    const userResponse = await fetch(`${process.env.SUPABASE_PROJECT_URL}/admin/users`, {
      method: 'GET',
      headers: {
    ...
    if (!userResponse.ok) {
      ...
      throw new Error(`Supabase GET /admin/users failed with ${userResponse.status}: ${errorText}`);
    }

    const { users } = await userResponse.json();

    for (const user of users) {
      const response = await fetch(`${process.env.SUPABASE_PROJECT_URL}/admin/users/${user.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
        },
      ...
      if (!response.ok) {
        ...
        console.error(`Error removing user ${user.id} — ${response.status}: ${errorText}`);
      } else {
...
const createRegUsers = async () => {
  try {
    const response = await fetch(`${process.env.SUPABASE_PROJECT_URL}/admin/users`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
    ...
    const response2 = await fetch(`${process.env.SUPABASE_PROJECT_URL}/admin/users`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        email: 'another.regular@user.com',
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Supabase POST /admin/users (regular user 1) failed with ${response.status}: ${errorText}`);
    }
    if (!response2.ok) {
      const errorText = await response2.text();
      throw new Error(`Supabase POST /admin/users (regular user 2) failed with ${response2.status}: ${errorText}`);
    }
  } catch (error) {
...
const createStaffUser = async () => {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Supabase POST /admin/users (staff user) failed with ${response.status}: ${errorText}`);
    }
  } catch (error) {
    console.log(`error: ${error.message}`);
  }
